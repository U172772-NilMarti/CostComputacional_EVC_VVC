...
def vvc_encoder(video_path, yuv_path):
    ...
    # Comando para codificar el video con VVC encoder
...
# Sobreescribir los datos en lugar de borrarlos completamente
#cursor.execute("TRUNCATE TABLE VIDEOS")
#cursor.execute("TRUNCATE TABLE VVC_ENCODER")
#cursor.execute("TRUNCATE TABLE EVC_ENCODER")
#cursor.execute("TRUNCATE TABLE EVC_FFMPEG_ENCODER")
...
# Recorremos la carpeta principal y sus subcarpetas
    ...
    if os.path.isdir(categoria_path):
        for video_file in os.listdir(categoria_path):
            if video_file.endswith('.mp4'):
                ...
                # Aplicar la función de codificación al vídeo
...
# DETINDRE EL PROCES NMON GLOBAL DEPEN DE L'OPCIO TRIADA, AQUI NO CAL ANALITZAR, JA HO FARE JO MANUAL
